% FREEFALLINGSINGLEBODY main script for running the free falling single 
%                       body test.
%
%                     REQUIRED VARIABLES:
%
...
% run the script containing the initial conditions for the single mass test
run(strcat(['./app/',Config.Simulator.modelFolderName,'/init_simulation.m']));

% create the initial state vector
chi_init = [wbc.fromTransfMatrixToPosQuat(Config.Model.w_H_b); Config.Model.baseVel];
...
disp('[freeFallingSingleBody]: integration started...')

forwardDynFunc = @(t,chi) forwardDynamicsSingleBody(t,chi,KinDynModel,Config);
...
disp('[freeFallingSingleBody]: integration ended.')
...
disp(['[freeFallingSingleBody]: integration time: ', ....
...
if Config.Simulator.showVisualizer
    ...
    Config.SimulationOutput.jointPos = '';
    Config.SimulationOutput.w_H_b    = [];
    Config.SimulationOutput.time     = time;
    
    for k = 1:size(state,1)

        w_H_b      = wbc.fromPosQuatToTransfMatr(state(k,1:7)');
        w_H_b_vect = w_H_b(:);
        
        Config.SimulationOutput.w_H_b = [Config.SimulationOutput.w_H_b w_H_b_vect];
    end 
else
...
if ~Config.Simulator.saveSimulationResults
    ...
    if exist('DATA','dir') && (exist(['./DATA/',Config.Visualization.dataFileName,'.mat'],'file') == 2)
        ...
        dataDir = dir('DATA');
        disp(['[freeFallingSingleBody]: removing file ','./DATA/',Config.Visualization.dataFileName,'.mat'])
